...
public class StroyManager : MonoBehaviour
{
    ...
    [SerializeField]
    private List<GameObject> DialogueLayouts;
        ...
        foreach (StoryStruct story in stories.stories)
        {
            if(story.storyID == id)
            {
                ...
                foreach (Dialogue dialogue in story.dialogues)
                {
                    if (dialogue.person != "")
                    {
                        Debug.Log("person = " + dialogue.person);
                        SetPersons(dialogue.person);
                    }
                    if (dialogue.dialogueText != "")
                    {
                        Debug.Log("dialogueText = " + dialogue.dialogueText);
                        ParseDialogue(dialogue.dialogueText);
                    }
                    if (dialogue.cloud != "")
                    {
                        SetClouds(dialogue.cloud);
                    }
                }
    ...
    private void SetClouds(string _bubble)
    {
        ...
        foreach(string cloud in _clouds)
        {
           ...
           // GameObject instCloud = null;
            switch (cloudOptions[2])
            {
                case "Left":
                   // instCloud = Instantiate(cloudObject, DialogueLayouts[0].transform);
                  // DialogueLayout.GetComponent<VerticalLayoutGroup>().childAlignment = TextAnchor.MiddleLeft;
                    break;
                case "Right":
                 //   instCloud = Instantiate(cloudObject, DialogueLayouts[1].transform);
    ...
    private void SetPersons(string _persons)
    {
       ...
       // Debug.Log("characters length = " + _characters.Length);
        //   foreach (string person in _characters)
        //   {
        //string[] personOptions = person.Split('#');
        string[] personOptions = _persons.Split('#');
        Debug.Log("person options = " + personOptions.Length);

        if(personOptions.Length == 3)
        {
            switch (personOptions[2])
            {
                case "Right": _num = 0;
                    break;
                case "Left": _num = 1;
        ...
        GameObject character;
            ...
            Debug.Log("num = " + _num + " anchors transform = " + Anchors[_num].transform.ToString());
